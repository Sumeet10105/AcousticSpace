...
def load_audio(
    ...
    try:
        ...
        logger.warning(f"torchaudio failed to load {filepath}: {str(e)}. Falling back to soundfile.")
        try:
            # Fallback to soundfile loading
            data, sr = sf.read(filepath)
            # sf.read returns shape [samples, channels] or [samples]
            waveform_np = data.astype(np.float32) if 'np' in globals() else data
            ...
            logger.error(f"soundfile fallback also failed for {filepath}: {str(sf_err)}")
            raise ValueError(f"Corrupted or invalid audio file: {filepath}. Error: {str(sf_err)}")
